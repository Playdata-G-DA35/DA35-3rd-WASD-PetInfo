...
class Kor_sp_preprocessing:

    def __init__(self, strpath, mname="m_bpe", vsize=10000, mtype="bpe"):
        self.strpath = strpath
        self.mname = mname
        self.vsize = vsize
        self.mtype = mtype
        ###########################################
        self.file_path = f"saved_models/{self.mname}.model"
        os.makedirs("saved_models", exist_ok=True)
        if os.path.isfile(self.file_path):
            print("파일존재")
        else:
            SentencePieceTrainer.Train(
                f"--input={self.strpath} --model_prefix=saved_models/{self.mname} --vocab_size={self.vsize} --model_type={self.mtype} --unk_id=0"
            )
        ########################################
        self.tokenizer = SentencePieceProcessor()
        self.tokenizer.load(f"saved_models/{self.mname}.model")



    def get_sp_tokenizer(self, txt):
        return self.tokenizer.EncodeAsPieces(txt)
...
if __name__ == "__main__" :
    ...
    kp = Kor_sp_preprocessing("data/petitions_corpus.txt")
    spt = kp.get_sp_tokenizer(txt)
    print(spt)
